...
bool eq(const ParameterTree& config_l, const ParameterTree& config_r)
{
...
// config_l += config_r
void add(ParameterTree& config_l, const ParameterTree& config_r, bool trow_on_override = true)
{
  for (auto &&key : config_r.getValueKeys())
  {
    if (config_l.hasKey(key))
      DUNE_THROW(RangeError,"config file addition failed due to duplucated key: " << key);
...
// config_l -= config_r
void diff(ParameterTree& config_l, const ParameterTree& config_r)
{
  ...
  for (auto &&section : config_l_copy.getSubKeys()) {
    ...
    if (config_l_copy.sub(section).getValueKeys().size() > 0
        or config_l_copy.sub(section).getSubKeys().size() > 0) // remove empty sections
      config_l.sub(section) = config_l_copy.sub(section);
  }
}

// get keys in vector
ParameterTree get_keys(const ParameterTree& config, const std::vector<std::string>& keys)
{
...
ParameterTree get_keys(const ParameterTree& config)
{
  return get_keys(config,config.getValueKeys());
}

// get sections in vector
ParameterTree get_sections(const ParameterTree& config, const std::vector<std::function<ParameterTree(const ParameterTree&)>>& sections)
{
  ParameterTree new_config;
...
ParameterTree get_grid(const ParameterTree& config)
{
...
ParameterTree get_initial(const ParameterTree& config)
{
...
ParameterTree get_diffusion(const ParameterTree& config, bool check_var_consistency = true)
{
  ParameterTree diffusion_config;
  if (check_var_consistency) {
    auto initial_config = get_keys(config.sub("initial"));
    diffusion_config = get_keys(config.sub("diffusion"),initial_config.getValueKeys());
  } else {
...
// check_var_consistency with respect to initial sections
ParameterTree get_operator(const ParameterTree& config, bool check_var_consistency = true)
{
  ParameterTree operator_config;
  if (check_var_consistency) {
    auto initial_config = get_keys(config.sub("initial"));
    operator_config = get_keys(config.sub("operator"),initial_config.getValueKeys());
  } else {
...
ParameterTree get_jacobian(const ParameterTree& config, std::vector<std::string> base_variables)
{
  ...
  for (auto &&var_i : base_variables)
  {
    for (auto &&var_j : base_variables)
    {
      ...
      if (found_i==std::string::npos)
        DUNE_THROW(RangeError, "Jacobian key '" << jac_key << "' does not contain its base key i '" << var_i << "'");
      auto found_j = jac_key.find(var_j);
      if (found_j==std::string::npos)
        DUNE_THROW(RangeError, "Jacobian key '" << jac_key << "' does not contain its base key j '" << var_j << "'");
      count++;
    }
  }
...
ParameterTree get_reaction(const ParameterTree& config, bool check_var_consistency = true)
{
  ParameterTree reaction_config;
  if (check_var_consistency) {
    auto initial_config = get_keys(config.sub("initial"));
    reaction_config = get_keys(config.sub("reaction"),initial_config.getValueKeys());
  } else {
    reaction_config = get_keys(config.sub("reaction"));
  }

  auto jacobian_config = get_jacobian(config.sub("reaction"),reaction_config.getValueKeys());
...
ParameterTree get_compartment_i(const ParameterTree& config, const std::string& name)
{
...
ParameterTree get_compartments(const ParameterTree& config)
{
...
ParameterTree get_model(const ParameterTree& config)
{
  auto compartments = get_compartments(config.sub("model"));

  std::vector<std::function<ParameterTree(const ParameterTree&)>> sections;
  for (auto &&compartment : compartments.sub("compartments").getValueKeys())
    sections.push_back([&](auto ini){return get_compartment_i(ini,compartment);});
